...
struct Api_NetworkReachability : Codable {
    
    let result : Res_NetworkRechability?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_NetworkRechability.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_NetworkRechability : Codable {
    let id : String?
    let store_id : String?
