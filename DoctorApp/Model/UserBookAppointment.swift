...
struct Api_UserBookAppointment : Codable {
    let result : [Res_UserBookAppoinment]?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_UserBookAppoinment].self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_UserBookAppoinment : Codable {
    let id : String?
    let user_id : String?
