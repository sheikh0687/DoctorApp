...
//  DoctorApp
//
//  Created by Techimmense Software Solutions on 07/11/24.
//

import Foundation

struct Api_RatingReview : Codable {
    let result : [Res_RatingReview]?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_RatingReview].self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_RatingReview : Codable {
    let id : String?
